<?php 
    ...
    if (!empty($order)) {
        ...
        $sql = "UPDATE `trade` SET `state_status`='$data->state_status',`suggested_bid`='$data->s_bid',`bid`=0,`crypto_var`=0,`on_actual_price`=0,`ricavo`=0,`future_sell_usdt`=0,`crypto_received`=0 WHERE `pricelevel`='$data->pricelevel' AND `crypto`='$data->crypto' AND `user` = '{$_SESSION['user_id']}'";
        ...
        if($runQuery){
            ...
            $total_possessed_crypto = $result[0]['possessed_crypto'] - $order['origQty']; 
